...
class PestControl {
  constructor (name = 'debug', options = {}) {
    ...
    if (!process.env.DEBUG) {
      ...
      const patterns = process.env.DEBUG.split(',').reduce((acc, text) => {
        text = text.trim()
        let blocked = false
        ...
        if (blocked) {
          acc.blocked.push(pattern)
        } else {
          acc.allowed.push(pattern)
        }

        return acc
      }, { allowed: [], blocked: [] })

      this.enabled = patterns.allowed.some((pattern) => pattern.test(this.name))
        && !patterns.blocked.some((pattern) => pattern.test(this.name))
    }
